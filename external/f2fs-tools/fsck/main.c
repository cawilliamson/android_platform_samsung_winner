...
void resize_usage()
{
	...
	MSG(0, "  -f fail-safe resize (Does not move main blocks) [default:1]\n");
...
void f2fs_parse_options(int argc, char *argv[])
{
	...
	if (!strcmp("fsck.f2fs", prog)) {
		...
		const char *option_string = "d:sft:V";

		c.func = RESIZE;
		c.fail_safe_resize = 1;
		while ((option = getopt(argc, argv, option_string)) != EOF) {
			int ret = 0;
			...
			case 'f':
				...
				c.fail_safe_resize = atoi(optarg);
				break;
			case 't':
				if (strncmp(optarg, "0x", 2))
					ret = sscanf(optarg, "%"PRIu64"",
		...
		if (c.fail_safe_resize)
			MSG(0, "Info: Fail-Safe resize mode on\n");
	} else if (!strcmp("sload.f2fs", prog)) {
		const char *option_string = "C:d:f:p:s:St:T:r:V";
