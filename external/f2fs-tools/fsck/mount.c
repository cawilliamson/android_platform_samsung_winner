...
static void restore_backup_meta(struct f2fs_super_block *sb)
{
	u64 from_blkaddr = get_sb(backupmeta_blkaddr);
	u64 target_segs = get_sb(segment_count_backupmeta);
	unsigned int blocks_per_seg = 1 << get_sb(log_blocks_per_seg);
	unsigned int seg_size = F2FS_BLKSIZE * blocks_per_seg;
	int i, ret;
	void *buf;
	...
	for (i = 0; i < target_segs; i++) {
		ret = dev_read(buf, F2FS_BLKSIZE * ((from_blkaddr + 2) +
					i * blocks_per_seg), seg_size);
		ASSERT(ret >= 0);
		ret = dev_write(buf, F2FS_BLKSIZE * (get_sb(segment0_blkaddr) +
					i * blocks_per_seg), seg_size);
		ASSERT(ret >= 0);
	}
...
static void check_backup_metadata()
{
	...
	for (i = 0; i < 2; i++) {
		if (dev_read_block(buf, i) < 0)
			return;
		sb = (struct f2fs_super_block *)(buf + F2FS_SUPER_OFFSET);

		if (!sanity_check_raw_super(sb, i * F2FS_BLKSIZE + F2FS_SUPER_OFFSET) &&
			get_sb(resize_state) & F2FS_META_BACKUP) {
			MSG(0, "Find backup metadata. restore backup meta!\n");
			restore_backup_meta(sb);
			return;
		}
	}
}
